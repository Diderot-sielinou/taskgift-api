...
pool.on("connect", (client) => {
  logger.info(`clent connected from `);
});
...
const initialzeDbSchema = async () => {
  ...
  try {
    logger.info(`Initializing data schema...`);
    ...
    await client.query(
      "CREATE INDEX IF NOT EXISTS idx_tasks_user_id ON tasks(owner_id)"
    );
    logger.info("index have been created")
  } catch (error) {
